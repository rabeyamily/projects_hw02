...
def get_random_quote(quotes: List[Dict[str, str]], tag: Optional[str] = None) -> Optional[Dict[str, str]]:
    """Retrieve a random quote, optionally filtered by tag."""
    if tag:
        filtered_quotes = [q for q in quotes if tag.lower() in q['tags'].lower()]
        return random.choice(filtered_quotes) if filtered_quotes else None
    ...
    while True:
        ...
        if choice == '1':
            ...
            if quotes:
                ...
                print("No quotes found matching the keyword.")
        elif choice == '3':
            break
        else:
